...
func TestListSerialization(t *testing.T) {
	...
	var err error
	assert := func() {
		var stringArr []string = list.Values()
		if actualValue, expectedValue := fmt.Sprintf("%s%s%s", stringArr[0], stringArr[1], stringArr[2]), "abc"; actualValue != expectedValue {
			t.Errorf("Got %v expected %v", actualValue, expectedValue)
		}
...
func benchmarkGet[V comparable](b *testing.B, list *List[V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Get(n)
		}
...
func benchmarkRemove[V comparable](b *testing.B, list *List[V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Remove(n)
		}
